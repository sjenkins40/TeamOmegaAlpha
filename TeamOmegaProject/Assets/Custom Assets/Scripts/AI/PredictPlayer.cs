...
public class PredictPlayer : BehaviorTreeNode {
	private float prediction = 10;
	private float counter = 0;
	private float distance;
	private UnityEngine.AI.NavMeshAgent navmesh;
	public PredictPlayer(UnityEngine.AI.NavMeshAgent navmesh, float distance, float prediction)
	{
	...
	public override int Act (BehaviorTree tree)
	{
		...
		if(!navmesh.pathPending)
		{
			...
			{
				counter = 0.3f;
				Vector3 goal = player.transform.position + player.velocity * prediction;
